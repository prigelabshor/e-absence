...
export default function ReportSickPage() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const record = {
            ...
            status: 'sakit' as RollCallStatus,
            notes: notes
        };

        try {
            await saveRollCall(institution, [record]);
            toast({ title: "Sukses", description: "Laporan sakit berhasil disimpan." });
            setSelectedStudent(undefined);
            setSelectedDormitory(undefined);
            setNotes("");
        } catch (error) {
            console.error("Error saving sick report:", error);
            toast({ title: "Error", description: "Gagal menyimpan laporan.", variant: "destructive" });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-3xl font-bold tracking-tight">Lapor Siswa Sakit</h1>
                <p className="text-muted-foreground">
                    Catat siswa atau santri yang sakit hari ini.
                </p>
            </div>

            <Card className="max-w-lg mx-auto">
                <CardHeader>
                    <CardTitle>Form Laporan Sakit</CardTitle>
                ...
                <form onSubmit={handleSubmit}>
                    <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea 
                                id="notes" 
                                placeholder="Misal: Demam, pusing, dll."
                                value={notes}
